...
- Prompt负责告诉模型“应该怎么回答”。
- `normalizeGeneratedReview()`负责修正异常返回，只有真实 API 调用失败才交给前端报错。
...
- 称号：4至9个汉字。
- 今日旁白：建议35至70个字符；过短会使用兜底旁白，过长会截断到安全长度。
- 建议：只保留1项次日可勾选任务，最长80个字符。
- 风格：Edge Function按55/25/15/5抽签，并把最终结果的`style`归一化为指定风格，避免模型字段小错误导致生成失败。
